...
export class TasksController {
  ...
  @Post()
  @Permissions(Permission.TASK_CREATE)
  async create(@Body() dto: CreateTaskDto, @CurrentUser() user: AuthUser): Promise<TaskDto>  {
    console.log(`[TASK CREATE] User: ${user.email}, Role: ${user.role}, Payload:`, dto);
    const task = await this.tasksService.create(dto, user.id, user.organizationId);
    await this.auditService.logTaskAction(user, 'CREATE', task);
  ...
  @Get()
  @Permissions(Permission.TASK_READ)
  async findAll(@CurrentUser() user: AuthUser): TaskDto[] {
    console.log(`[TASK READ ALL] User: ${user.email}, Role: ${user.role}`);
    let tasks: TaskDto[];
    if (user.role === 'OWNER' || user.role === 'ADMIN') {
      tasks = await this.tasksService.findAllForOrganization(user.organizationId);
    } else {
      tasks = await this.tasksService.findAllForUser(user.id, user.organizationId);
    }
    await this.auditService.logTaskAction(user, 'READ_ALL');
  ...
  @Get(':id')
  @Permissions(Permission.TASK_READ)
  async findOne(@Param('id') id: string, @CurrentUser() user: AuthUser): TaskDto {
    console.log(`[TASK READ] User: ${user.email}, Role: ${user.role}, Task ID: ${id}`);
    const task = await this.tasksService.findOne(id);

    if (user.role === 'OWNER' || (user.role === 'ADMIN' && task.organizationId === user.organizationId)) {
      await this.auditService.logTaskAction(user, 'READ', task);
  ...
  @Put(':id')
  @Permissions(Permission.TASK_UPDATE)
  async update(@Param('id') id: string, @Body() dto: UpdateTaskDto, @CurrentUser() user: AuthUser): TaskDto {
    ...
    if (user.role === 'OWNER' || (user.role === 'ADMIN' && task.organizationId === user.organizationId)) {
      ...
      await this.auditService.logTaskAction(user, 'UPDATE', updated);
  ...
  @Delete(':id')
  @Permissions(Permission.TASK_DELETE)
  async remove(@Param('id') id: string, @CurrentUser() user: AuthUser): void {
    console.log(`[TASK DELETE] User: ${user.email}, Role: ${user.role}, Task ID: ${id}`);
    const task = await this.tasksService.findOne(id);

    if (user.role === 'OWNER' || (user.role === 'ADMIN' && task.organizationId === user.organizationId)) {
      await this.tasksService.remove(id);
      await this.auditService.logTaskAction(user, 'DELETE', task);
